...
function ImageCard({ item, className = "" }: any) {
  return (
    <Link
      href={item.url}
      target="_blank"
    ...
    >
      <img
        src={item.image}
        alt={item.title}
        className="absolute scale-110 inset-0 w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
      />
      <div className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-black to-transparent" />
      <span className="absolute bottom-4 left-4 text-white font-semibold text-2xl">
        {item.title}
      </span>
    </Link>
  );
